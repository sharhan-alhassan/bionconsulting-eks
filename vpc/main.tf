...
resource "aws_internet_gateway" "core_src" {
...
resource "aws_nat_gateway" "core_src" {
  count         = length(var.private_subnets)
  allocation_id = element(aws_eip.nat.*.id, count.index)
  subnet_id     = element(aws_subnet.public.*.id, count.index)
  depends_on    = [aws_internet_gateway.core_src]
...
resource "aws_route" "public" {
  ...
  gateway_id             = aws_internet_gateway.core_src.id
}
...
resource "aws_route" "private" {
  ...
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = element(aws_nat_gateway.core_src.*.id, count.index)
}
...
resource "aws_flow_log" "core_src" {
